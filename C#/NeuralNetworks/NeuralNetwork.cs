...
namespace NeuralNetworks
{
    public class NeuralNetwork
    {
        ...
        {
            var result = new double[inputs.Length, inputs[0].Length];
            ...
            for (int column = 0; column < inputs[0].Length; column++)
            {
                ...
                var error = 0.0;
                for (int row = 1; row < inputs.Length; row++)
                {
        ...
        private double Backpropagation(double exprected, params double[] inputs)
        {
            ...
            for (int j = Layers.Count - 2; j >= 0; j--)
            {
                ...
                for (int i = 0; i < layer.NeuronCounts; i++)
                {
                    var neuron = layer.Neurons[i];

                    for (int k = 0; k < previousLayer.NeuronCounts; k++)
                    {
        ...
        private void CreateOutputLayer()
        {
            ...
            {
                var neuron = new Neuron(lastLayer.NeuronCounts, NeuronType.Output);
        ...
        private void CreateHiddenLayers()
        {
            for (int j = 0; j < Topology.HiddenLayers.Count; j++)
            {
                ...
                {
                    var neuron = new Neuron(lastLayer.NeuronCounts);
